...
public:
  ...
  void waitForAccessLogConnectionAndStream() {
  ...
  void waitForAccessLogRequest(const std::string& expected_request_msg_yaml) {
    ...
    // Clear request_id which we do not control.
    request_msg.mutable_http_logs()->mutable_log_entry(0)->mutable_request()->clear_request_id();
...
TEST_P(AccessLogIntegrationTest, BasicAccessLogFlow) {
  testRouterNotFound();
  waitForAccessLogConnectionAndStream();
